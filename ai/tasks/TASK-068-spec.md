...
1. **SATISFIED.** Sentry is verifiably initialized before `main.jsx`'s own import graph evaluates, using
   whichever mechanism §2.0's verification confirms is current best practice — not merely inferred from
   source-file statement ordering, and **checked mechanically against the production build, not dev-server
   behavior** (DRAFT-7, round-6's non-blocking #4) — **and §2.0's decision table is fully populated, not left
   as an unfilled checklist.** Server init happens via `server/instrument.js`, imported from `server/app.js`
   ...
   or the server from starting (DRAFT-7, new — round-6's non-blocking #3)** — F11 confirmed via forced-throw
   test (server) and by construction (client, identical try/catch pattern); F12 confirmed mechanically
   against the actual production bundle's byte offsets, not source-level argument alone (§6 F11/F12).
...
11. **Duplicate-error verification — SATISFIED (round-9 required test, see §8).** A single
    deliberately-thrown React render error (caught by `ErrorBoundary`) produces **exactly one Sentry event
    attributable to that exception** (via the server-side `/api/client-errors` path), with no second event
    independently generated by the client SDK's global error/`unhandledrejection` handlers (DRAFT-4,
    round-3's P1 #9 — tightened from "actionable event," which was ambiguous about whether two
    differently-fingerprinted events would count as one or two). **Confirmed against a real deployed Vercel
    Preview** (commit `cbc7b6a`, authenticated): exactly one event, landing only in `kitchen-keeper-server`,
...
13. **SATISFIED — 10/10 delivered.** Serverless delivery under multiple invocations. Trigger a known batch
    ...
    window, recorded in the implementation notes rather than left as "eventually" (DRAFT-4, round-3's P1
    #17 — a specific count is unambiguous; "the expected event count" was not). Confirmed against the real
    `staging` Preview: all 10 unique indices present, ~7s delivery window, release `70f741d53a2f` (§6, step I).
...
**F11. Init-failure-doesn't-block-boot check — DONE (server confirmed live; client confirmed by
construction).** While preparing this check, found `Sentry.init()` was **not** actually wrapped in a
try/catch on either side — a real gap, not yet a confirmed bug at that point, since criterion 1 explicitly
requires this. Fixed both `client/src/instrument.js` and `server/instrument.js` to wrap `Sentry.init()` in
`try { ... } catch { /* must never block boot */ }`. **Server-side verified empirically**: temporarily
forced a throw (`if (process.env.__TASK068_FORCE_INIT_THROW) throw ...` inside the try block), ran `node
index.js` locally, confirmed `GET /api/health` still returned `200` — proof the fix works, since without it
the throw would have aborted `app.js`'s entire module evaluation before `app.listen()` ever ran. Reverted
the test hook immediately after. **Client-side not separately browser-tested** — the fix is the identical
pattern already proven server-side, and "a caught exception doesn't halt subsequent code in the same
module" is basic JS `try/catch` semantics, not something specific to Sentry or this codebase that needs
independent empirical re-proof (criterion 1).

**F12. Mechanical init-ordering proof against the production build — DONE.** Built the client for
production (`npm run build`) and inspected the actual output bundle (`client/dist/assets/index-*.js`) for
byte offsets of `Sentry.init()`'s own option keys (`enableLogs`, `dataCollection` — unique to this code)
versus `main.jsx`'s `ReactDOM.createRoot()` call and `ChatPage`'s own rendered content. Result:
`enableLogs`/`dataCollection` at offsets 48,616/56,504; `createRoot` at 233,448; `ChatPage` content at
671,132 — Sentry's init code sits well before either, in the file Rollup actually produces. Since Rollup
bundles ES modules into one file with top-level statements executing in their literal file order (the same
guarantee source-level ESM semantics provide, carried through bundling), this is direct evidence from the
production artifact itself, not an argument from source-level reasoning alone (criterion 1).
...
**I. Serverless delivery smoke test — DONE, 10/10 delivered (DRAFT-7, relabeled per round-6's §15 — this
demonstrates delivery isn't universally broken under this deployment's flush behavior, not a statistical
reliability guarantee).** Added a temporary `GET /api/__task068_burst_test/:n` route, pushed, then ran
`curl` 10 times (indices 1-10) directly against the deployed Preview (no browser needed — separate Vercel
Function invocations per request, real serverless suspend/resume behavior, not one warm local process).
Confirmed via Sentry's per-issue event search (`message:"event N/10"` for each N) that **all 10 unique
indices arrived** — total event count was 12, explained exactly by 2 extra manual verification pings against
index 1 before the formal loop (3 total for index 1, 1 each for indices 2-10 = 12). **Deployment SHA**:
`70f741d53a2f` (release tag, matches pushed commit `70f741d`). **Environment**: `staging`. **Delivery
window**: ~7 seconds, first event 7:21:56 PM UTC, last 7:22:03 PM UTC, 2026-08-17. Temporary route fully
reverted and pushed immediately after (criterion 13).
...
**What this needed to actually run**: a Vercel Preview deployment with Sentry's env vars actually
configured. Connor added all seven vars (`SENTRY_DSN`/`VITE_SENTRY_DSN`,
`SENTRY_ENVIRONMENT`/`VITE_SENTRY_ENVIRONMENT` = `staging`, `SENTRY_ORG`/`SENTRY_PROJECT`/
`SENTRY_AUTH_TOKEN`, `SENTRY_AUTH_TOKEN` marked sensitive) to Vercel's Preview scope. Implementation was
committed and pushed (`dfa8d5a`), a temporary authenticated-render-error trigger added to `ChatPage.jsx`
(gated behind `?__task068_throw=1`, the app's signed-in index route) and pushed (`cbc7b6a`) to get a fresh
Preview build.

### Round-9 required test — RESULT: PASSED

Signed in on the real deployed Preview (`kitchen-keeper-git-staging-connorsharpes-projects.vercel.app`),
triggered the deliberate render error, confirmed via the Network tab that `POST /api/client-errors`
returned `204` (reached the server, passed `clerkAuth`, completed normally). Checked the Sentry dashboard:

- **Exactly one issue**, landing only in `kitchen-keeper-server` (`?(clientErrors)` transaction) — **zero**
  events in `kitchen-keeper-client`, confirmed both in the unfiltered feed and the client-project-scoped
  view. The 2-event count on that one issue is from deliberately triggering it twice during Network-tab
  troubleshooting (an intentional reload), not a duplicate-capture mechanism firing twice.
- `environment: staging` ✓, `release: cbc7b6aa0712` (matches the pushed commit) ✓.
- Context fields present exactly as `captureExceptionSafely()`'s closed shape specifies: `deploy: cbc7b6a`,
  `requestId: a08da056`, `householdId: 1`, `userId: user_3FVuvJJGq9W65mQ1SrVwLaz48wS`, plus
  `client.componentStack` (full stack visible) and `client.originalStack` (present as a field) in Contexts.

**One observation, not a blocker**: `client.originalStack`'s *value* displayed as `[Filtered]` in the Sentry
UI, along with one breadcrumb's `arguments`. This is Sentry's own default server-side data scrubber (Security
& Privacy → Data Scrubber), not this task's `validateTelemetryShape()`/shape-allowlist code — that allowlist
only governs `logEvent()`'s path (§2.3a), not `captureExceptionSafely()`'s `clientContext`. Worth knowing
about if `originalStack` content is ever needed for real debugging (check the project's Data Scrubber
settings), but not a functional problem — the field is present and correctly separated from server-side
stack data, satisfying what criterion 12/16's field-naming and criterion 11's "context fields present"
language actually require.

Temporary trigger reverted and pushed (`467b0f9`) immediately after the test. `git diff` confirms
`ChatPage.jsx` matches its pre-test state exactly.

**This satisfies round-9's required test in full**: exactly one event, correct `environment`/`release`,
correct closed-shape context fields present, zero independent automatic client-side capture on the real
production build. Per round-9's own stated expectation, this should not require a further broad review
round — the two architectural options that would have required more implementation work (disabling an
integration, or building a `beforeSend` filter) are both confirmed unnecessary.
