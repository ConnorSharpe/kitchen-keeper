...
**RESOLVED (2026-07-21, same day): Clerk support replied and identified the real root cause — a stale Google OAuth client secret, not an Account Portal bug.**
Clerk's support (Ryan) traced production request/email logs directly and found three separate things, only one of which was an actual bug:
1. **Root cause of the sign-in failure**: Google's OAuth token endpoint was returning `401 invalid_client` on the token exchange — the client secret Clerk had stored for the production Google OAuth client (`303977838810-e03j0d723deukb18ckbd09erralqikmr.apps.googleusercontent.com`, project "Kitchen Keeper Production") no longer validated against Google, even though the client ID itself was correct (confirmed via `clerk config pull --instance ins_3GbWJwo4GVlGAD7lyMCMgXmMotn`). This explains everything above: Clerk's backend genuinely was creating `sign_in`/`sign_up` objects successfully (hence `created`/`code_sent` states with no failure event), but could never finish because the final Google token exchange silently failed one layer down — not a bug in Clerk's Account Portal frontend after all.
   - **Fix (no code/config in this repo — entirely Google Cloud Console + Clerk Dashboard)**: generated a fresh OAuth client secret on the existing "Kitchen Keeper Production" client in Google Cloud Console (`console.cloud.google.com/apis/credentials`, project `303977838810`) and saved it into Clerk Dashboard → production instance (`ins_3GbWJwo4GVlGAD7lyMCMgXmMotn`) → User & Authentication → Social Connections → Google → Client Secret. **User confirmed live: Google OAuth sign-in on `kitchenkeeper.kitchen` now completes successfully.**
   - The stale/original secret (ends `...s7Br`, created Jul 16) was left enabled in Google Cloud Console alongside the new one — safe to delete later, not urgent.
2. **Email-code sign-up "never arrives"**: Clerk's logs show both attempts were sent successfully and Gmail acknowledged receipt (`250 2.0.0 OK`) — not a Clerk-side failure. Not independently retested since the OAuth fix (Google sign-in path is confirmed working, which was the priority); if email-code sign-up is still needed, check Gmail's All Mail/filters or try a different recipient.
3. **`origin_invalid` errors seen from `https://kitchenkeeper.vercel.app`**: expected behavior, not a bug. Production Clerk keys only accept requests from the verified `kitchenkeeper.kitchen` domain. Vercel's default `.vercel.app` alias points at the same Production deployment (and thus the same `pk_live_`/`sk_live_` keys), so it gets correctly rejected. **Always test the production auth flow at `https://kitchenkeeper.kitchen`, never the `.vercel.app` URL.**

**The "Clerk Dashboard sign-up hardening" deployment prerequisite is now fully done** — production instance exists, DNS/SSL/mail/OAuth-app all complete, and sign-in itself works end-to-end. Nothing further needed here barring new issues.
...
# Remaining Work
1. **Deployment Prerequisites — 2 of 3 resolved, 1 still fully open, non-code** (per [TASK-037-spec.md](../tasks/TASK-037-spec.md), "Deployment Prerequisites" section) — required before ever flipping `public_ai_access_enabled` to `true` in production:
   ...
   - ~~Clerk Dashboard sign-up hardening~~ — **resolved 2026-07-21**: root cause was a stale Google OAuth client secret (see "Clerk Production Migration" section above), fixed by regenerating the secret in Google Cloud Console and saving it in Clerk Dashboard. Google OAuth sign-in on `kitchenkeeper.kitchen` confirmed working live by the user.
...
TASK-037's code is implemented, verified, committed, and pushed — that part is fully done. The Clerk production migration is now **fully resolved** — Google OAuth sign-in on `kitchenkeeper.kitchen` works end-to-end (see "Clerk Production Migration" section above). The only remaining deployment prerequisite is **OpenAI prepaid billing** (switch to prepaid credits, auto-recharge off) — not started, no diagnosis needed, just an account setting outside this repo. Once that's done, all three Deployment Prerequisites are clear and `public_ai_access_enabled` can be flipped to `true` in production whenever desired.
