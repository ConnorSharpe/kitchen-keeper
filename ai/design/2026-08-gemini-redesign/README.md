...
| `13-logo-concepts.png` | Logo/wordmark (not a screen) | N/A — plain background | **Approved direction.** Three variations on one canvas: (1) a single bold spoon silhouette paired with the "KITCHEN KEEPER" wordmark; (2) the icon alone; (3) wordmark alone. Landed here after two rejected rounds — a house+pantry-shelf+leaf combination mark was too detailed to survive favicon-scale rendering (verified by downscaling to real 16px/32px and visually inspecting), and a mason-jar+leaf version read as a gardening app rather than a food app. The spoon alone tested cleanly legible at 16px and was implemented directly: `client/public/favicon.svg`, `client/public/icon.svg`, and `client/public/apple-touch-icon.png` were all replaced with this mark, recolored to the app's canonical design tokens (`--kk-green-800` #254832 on `--kk-cream-100` #fdfaf5, or reversed) rather than Gemini's approximate on-image colors — check `client/src/index.css` for the current token values if they've since changed |
