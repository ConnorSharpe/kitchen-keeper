    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Join a household</h1>
          <p className="text-sm text-gray-500">Sign in or create an account to accept your invitation.</p>
        </div>
        <SignIn routing="hash" afterSignInUrl={`/join?code=${code}`} afterSignUpUrl={`/join?code=${code}`} />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4">
      <div className="bg-white border border-gray-200 rounded-2xl p-8 max-w-sm w-full text-center space-y-4">
        ...
        {joining && (
          <p className="text-sm text-gray-500">Joining household…</p>
        )}
        ...
        {!joining && (
          <form
            onSubmit={(e) => {
              ...
              setJoining(true);
              setError('');
              api.post('/api/household/join', { code })
