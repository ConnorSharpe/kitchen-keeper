...
router.get('/', async (req, res) => {
  const expiringWithin =
    req.query.expiring != null ? Number(req.query.expiring) : undefined;
  const items = await pantryService.getAll(req.user.householdId, {
    expiringWithin,
  });
  res.json({ items });
});
...
router.get('/waste-saved', async (req, res) => {
  const count = await pantryService.getWasteSaved(
...
router.post('/bulk', validate(bulkCreateSchema), async (req, res) => {
  const items = await pantryService.bulkCreate(
    req.user.householdId,
    req.body.items
  );
...
router.patch('/:id/use', async (req, res) => {
  const result = await pantryService.markUsed(
    req.user.householdId,
    Number(req.params.id)
  );
  if (ownershipError(result, res)) return;
...
router.patch('/:id/freeze', async (req, res) => {
  const result = await pantryService.toggleFreeze(
    req.user.householdId,
    Number(req.params.id)
  );
  if (ownershipError(result, res)) return;
...
router.post('/:id/split', validate(splitSchema), async (req, res) => {
  const result = await pantryService.splitItem(
    req.user.householdId,
    Number(req.params.id),
    req.body
  );
  ...
  if (result.status === 'invalid_quantity') {
    return res
      .status(400)
      .json({ error: 'Split quantity exceeds available quantity' });
  }
  if (result.status === 'no_servings_configured') {
    return res
      .status(400)
      .json({ error: 'This item has no servings-per-unit value set' });
  }
...
router.delete('/:id', async (req, res) => {
  const result = await pantryService.remove(
    req.user.householdId,
    Number(req.params.id)
  );
  if (ownershipError(result, res)) return;
