...
router.get('/', async (req, res) => {
  ...
  const items = await pantryService.getAll(req.user.id, { expiringWithin });
  res.json({ items });
});
...
router.get('/waste-saved', async (req, res) => {
  const count = await pantryService.getWasteSaved(req.user.id, req.query.since);
  res.json({ count });
});

// POST /api/pantry
router.post('/', validate(createSchema), async (req, res) => {
  const item = await pantryService.create(req.user.id, req.body);
...
router.post('/bulk', validate(bulkCreateSchema), async (req, res) => {
  const items = await pantryService.bulkCreate(req.user.id, req.body.items);
  res.status(201).json({ items });
});

// PATCH /api/pantry/:id
router.patch('/:id', validate(updateSchema), async (req, res) => {
  const result = await pantryService.update(req.user.id, Number(req.params.id), req.body);
...
router.patch('/:id/use', async (req, res) => {
  const result = await pantryService.markUsed(req.user.id, Number(req.params.id));
  if (ownershipError(result, res)) return;
  res.json({ ok: true });
});
...
router.patch('/:id/freeze', async (req, res) => {
  const result = await pantryService.toggleFreeze(req.user.id, Number(req.params.id));
  if (ownershipError(result, res)) return;
  res.json({ item: result.item });
});
...
router.delete('/:id', async (req, res) => {
  const result = await pantryService.remove(req.user.id, Number(req.params.id));
  if (ownershipError(result, res)) return;
  res.json({ ok: true });
});
