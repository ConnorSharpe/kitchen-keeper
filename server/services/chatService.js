...
export async function savePair(userId, userMessage, assistantReply) {
  await db.transaction(async (tx) => {
    await tx.insert(chatMessages).values({ userId, role: 'user',      content: userMessage });
    await tx.insert(chatMessages).values({ userId, role: 'assistant', content: assistantReply });
  });
}
...
export async function trimHistory(userId, keepLast) {
  const all = await db
